...
def hooray(times_requested):
    if times_requested > 1:
        hooray(times_requested - 1)
    print("Hooray!", times_requested)


def yahoo(times_requested):
    if times_requested > 0:
        yahoo(times_requested - 1)
    print("Yahoo!", times_requested)
...
# 1) Create a function that uses recursion to sum all the numbers in a list.
...
def sum_list(list_of_numbers):
    if len(list_of_numbers) > 1:
        return list_of_numbers[0] + sum_list(list_of_numbers[1:])
    else:
